...
```{r}
intros.info |>
  summarize(prop.small.intros = sum(intro.size < 100000)/n())
```
...
```
```{r}
ggsave(filename = "C:/Users/jholland/Box/nNIL genotype data Jim and Tao/Output/Histogram of Introgression Sizes No Filtering.tiff", dpi = 300, width = 10, height = 8, units = "in")
```
...
```
```{r}
ggsave(filename = "C:/Users/jholland/Box/nNIL genotype data Jim and Tao/Output/Histogram of Introgression Proportions of Chromosomes No Filtering.tiff", dpi = 300, width = 10, height = 8, units = "in")
```
...
line.summary = intros.info %>%
  ...
  summarize(N.blocks.unfiltered = n(),
            Total.Intro.Sizes = sum(intro.size),
            Total.Intro.Prop = Total.Intro.Sizes/V4.total.size) 

summary(line.summary$Total.Intro.Prop)
```

```{r}
line.summary %>%
  ggplot(aes(x = Total.Intro.Prop)) +
  geom_histogram(fill = 'blue') +
  xlab("Proportion of genome introgressed per line") +
  geom_vline(xintercept = mean(unlist(line.summary$Total.Intro.Prop)), color = "red") +
  geom_vline(xintercept = median(unlist(line.summary$Total.Intro.Prop)), color = "red", linetype = 2) +
  theme_minimal()
```
...
```{r}
sum(mismatch.by.NIL$all.intros.match.ped)
```
...
```{r}
sum(mismatch.by.NIL.filtered$all.intros.match.ped)
```
...
```
```{r}
#ggsave(filename = "C:/Users/jholland/Box/nNIL genotype data Jim and Tao/Output/Histogram of Introgression Sizes Only Good Founder Matches.tiff", dpi = 300, width = 10, height = 8, units = "in")
```
...
```
```{r}
#ggsave(filename = "C:/Users/jholland/Box/nNIL genotype data Jim and Tao/Output/Histogram of Introgression Proportions of Chromosomes.tiff", dpi = 300, width = 10, height = 8, units = "in")
```
...
line.summary = line.summary |>
  mutate(all.intros.match.ped = ifelse(is.na(all.intros.match.ped), F, all.intros.match.ped)) #NA values are F

line.summary %>%
  filter(all.intros.match.ped) |>
  pull(Total.Intro.Prop) |>
  summary()
```
...
line.summary %>%
  ...
  geom_histogram(fill = 'blue') +
  xlab("Proportion of genome introgressed per line") +
  geom_vline(xintercept = mean(unlist(line.summary[line.summary$all.intros.match.ped,'Total.Intro.Prop'])), color = "red") +
  geom_vline(xintercept = median(unlist(line.summary[line.summary$all.intros.match.ped,'Total.Intro.Prop'])), color = "red", linetype = 2) +
  theme_minimal()
```

```{r}
#ggsave(filename = "C:/Users/jholland/Box/nNIL genotype data Jim and Tao/Output/Histogram of Proportion of Whole Genome Introgressed by Line.tiff", dpi = 300, width = 10, height = 8, units = "in")
```
...
```{r}
line.summary = line.summary |>
  select(Line, ped.donor, best.matches, all.intros.match.ped, N.blocks, N.mismatch.blocks, N.NA.blocks, N.donor.matches, Total.Intro.Sizes, Total.Intro.Prop)
write.csv(line.summary, file = paste0(base.path, "nNIL_data_supplement/File_S06.nNIL_donor_match_summary.csv"), row.names = F, quote = F)
```
