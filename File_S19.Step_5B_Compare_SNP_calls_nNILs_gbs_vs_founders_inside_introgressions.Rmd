...
```
# Visualize introgression size distributions 
...
intros.info.for.plots  = intros.info  %>%
  mutate(intro.size.Mbp = intro.size/1000000,
...
```
```{r}
ggsave(filename = "C:/Users/jholland/Box/nNIL genotype data Jim and Tao/Output/Histogram of Introgression Sizes.tiff", dpi = 300, width = 10, height = 8, units = "in")
```
...
line.summary = intros.info %>%
  group_by(Line) %>%
  summarize(Total.Intro.Sizes = sum(intro.size),
...
```{r}
write.csv(intros.info, file = paste0(base.path, "nNIL_data_supplement/File_S20.Individual_Introgressions_gbs_donor_matches.csv"), row.names = T, quote = F)
```
...
write.csv(line.summary, file = paste0(base.path, "nNIL_data_supplement/File_S21.nNIL_donor_match_summary.csv"), row.names = F, quote = F)
```
