...
How many NILs have at least one mismatched introgression?
```{r}
mismatch.by.NIL |>
...
mismatch.by.NIL |>
  ...
  nrow()
```
How many NILs have no certain matches (all blocks have NA donor match)?
```{r}
mismatch.by.NIL |>
  filter(N.donor.matches == 0 & N.mismatch.blocks == 0) |>
  nrow()
```
Save the file with details of individual introgressions and their match probabilities
```{r}
...
Save the file with details of line introgression donor matches
```{r}
write.csv(mismatch.by.NIL, file = paste0(base.path, "nNIL_data_supplement/File_S22.nNIL_donor_match_summary.csv"), row.names = T, quote = F)
```
