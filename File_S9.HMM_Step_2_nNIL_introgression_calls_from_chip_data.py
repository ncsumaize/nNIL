...
sys.path.insert(0, 'Q:/.shortcut-targets-by-id/1FP9BlrAC2EltlqKM3ounRs2COTiTK12G/nNIL genotype data Jim and Tao/Code')
import callIntrogressions as ci
...
chip_header = pd.read_excel("Data/Chip data of NAM parents and nNILs.xlsx",  header = None, nrows = 1)
chip_lines = chip_header.loc[0,9:] #get the line names

chip = pd.read_excel("Data/Chip data of NAM parents and nNILs.xlsx", header = 0, skiprows = 1)
...
chipV3pos = pd.read_table("Output/nNIL_chip_SNP_positions_V3_6col.bed", header = None)
chipV3pos.columns = ['chr', 'startV3', 'pos_V3',  'name', 'score', 'strand']
chipV3pos.drop(columns = ['startV3', 'score', 'strand'], inplace = True)
chipV4pos = pd.read_table("Data/nNIL_chip_SNP_positions_converted_to_V4.bed", header = None)
...
gbsMarkers = pd.read_table('Output/nNIL_filtered_marker_list', header = None)
...
chipMarkersDF.to_csv("Output/nNIL chip markers and closest GBS markers.csv",  index = False)
...
# #reorder columns
# newcols = ['nir', 'scer', 'r', 'B73homozIntroMean', 'B73hetMean', 'DonorhetMean', 'DonorhomozIntroMean',
#            'NILhetMean', 'NILhomozIntroMean', 'PercNILsNoIntro', ]
# summaryDF = summaryDF[newcols]



#Write the summaryDF to a csv so we can visualize results in R
summaryDF.to_csv('Output/nNIL_chipdata_HMMgridSearchSummary.csv', index=False)
...
finalModel.to_csv('Output/nNIL_chipdata_HMM_introgressionCalls.csv')
...
NIL_calls_projected.to_csv('Output/nNIL_chipdata_HMM_introgressionCalls_project_to_GBS_markers.csv')
